  ...
  namespace detail {
    ...
    inline void write_help(std::ostream& out) {
      std::println(
        ...
        "\n"
        "Reads CSV-encoded event lines from stdin, drives the order book through an\n"
        "event-driven runtime (reader → matcher → writer agents joined by SPSC queues),\n"
        "writes trade and fill events to stdout, and reports parse errors on stderr.\n"
        "\n"
        "Input grammar (one message per line):\n"
        "  ADD,<orderid>,<side>,<quantity>,<price>,<tif>   add order (side: BUY | SLL; tif: GTC | IOC | FOK)\n"
        "  CXL,<orderid>                                    cancel order\n"
        "\n"
        "Output grammar:\n"
        "  TRD,<aggressive-id>,<resting-id>,<quantity>                 TradeEvent\n"
        "  RST,<orderid>,<filled>,<resting>                           order accepted, residue rests\n"
        "  FIL,<orderid>,<filled>                                     order fully filled\n"
        "  CAN,<orderid>,<filled>,<cancelled>,<cause>                   order cancelled; cause: USR | IOC | FOK\n"
        "  REJ,<orderid>,<code>                                       order rejected; code: DUP | UNK | IQT | IPR | "
        "IID\n"
        "Flags:\n"
